...
function parseGptReportResponse(responseText, projectDetails) {
  try {
    // Safety check - ensure we have text to parse
    if (!responseText || typeof responseText !== 'string') {
      console.error('Invalid GPT response:', responseText);
      return projectDetails.issues.map(issue => ({
        issue: issue.description,
        actionsTaken: issue.actionsTaken,
        detailedAnalysis: "Unable to generate analysis. Please try again.",
        legalContext: "",
        relevantClauses: ["Unable to determine relevant clauses."],
        clauseExplanations: [],
        recommendations: ["Please consult with a legal professional regarding this issue."],
        potentialOutcomes: "",
        timelineSuggestions: "",
        riskAssessment: ""
      }));
    ...
    projectDetails.issues.forEach((issue, index) => {
      if (index >= issueResponses.length) {
        // Fallback if we don't have enough responses for all issues
        analysisArray.push({
          issue: issue.description,
          actionsTaken: issue.actionsTaken,
          detailedAnalysis: "Unable to generate complete analysis for this issue.",
          legalContext: "",
          relevantClauses: generateFallbackClauses(projectDetails.contractType, issue.description),
          clauseExplanations: [],
          recommendations: ["Seek professional legal advice specific to your contract situation."],
          potentialOutcomes: "",
          timelineSuggestions: "",
          riskAssessment: ""
        });
      ...
      try {
        ...
        const detailedAnalysis = extractSection(responseForIssue, "Detailed Analysis", true) || 
                                "Analysis could not be extracted from the response.";
        ...
        analysisArray.push({
          ...
          clauseExplanations: clauseExplanations.length > 0 ? clauseExplanations : [],
        ...
        console.error('Error extracting sections for issue:', sectionError);
        // Add fallback analysis for this issue
        analysisArray.push({
          issue: issue.description,
          actionsTaken: issue.actionsTaken,
          detailedAnalysis: "Error extracting analysis information.",
          legalContext: "",
          relevantClauses: generateFallbackClauses(projectDetails.contractType, issue.description),
          clauseExplanations: [],
          recommendations: ["Seek professional legal advice specific to your contract situation."],
    ...
    console.error('Error in parseGptReportResponse:', error);
    // Return a fallback analysis array to avoid returning invalid data
    return projectDetails.issues.map(issue => ({
      issue: issue.description,
      actionsTaken: issue.actionsTaken,
      detailedAnalysis: "An error occurred while analyzing this issue.",
      legalContext: "",
      relevantClauses: ["Unable to determine relevant clauses."],
      clauseExplanations: [],
...
function splitResponseByIssues(text, issueCount) {
  try {
    ...
    console.error('Error splitting response by issues:', error);
    // Return the whole text as a single issue if splitting fails
    return [text];
  }
}
...
function extractListItems(text, sectionName) {
  try {
    ...
    console.error(`Error extracting list items for section "${sectionName}":`, error);
    return [];
  }
}
...
function generateFallbackClauses(contractType, issueDescription) {
  try {
    ...
    if (contractType && contractType.includes('JCT')) {
    ...
    if (contractType && contractType.includes('NEC')) {
    ...
    console.error('Error generating fallback clauses:', error);
    return ['Contract Clauses (Please consult your contract)'];
  }
}